"""Environment loading for ForgeX application configuration."""
...
LLM_TIMEOUT_ENV_VAR = "FORGEX_LLM_TIMEOUT_SECONDS"
LEGACY_LLM_TIMEOUT_ENV_VAR = "PROMPTFORGE_LLM_TIMEOUT_SECONDS"
DEFAULT_LLM_TIMEOUT_SECONDS = 180.0
LLM_MAX_ATTEMPTS_ENV_VAR = "FORGEX_LLM_MAX_ATTEMPTS"
LEGACY_LLM_MAX_ATTEMPTS_ENV_VAR = "PROMPTFORGE_LLM_MAX_ATTEMPTS"
DEFAULT_LLM_MAX_ATTEMPTS = 2
LLM_RETRY_BACKOFF_ENV_VAR = "FORGEX_LLM_RETRY_BACKOFF_SECONDS"
LEGACY_LLM_RETRY_BACKOFF_ENV_VAR = "PROMPTFORGE_LLM_RETRY_BACKOFF_SECONDS"
DEFAULT_LLM_RETRY_BACKOFF_SECONDS = 1.0


def load_environment(env_file: str | Path | None = None) -> Path:
    """Load ForgeX's dotenv file without overriding process variables."""
...
def llm_timeout_seconds() -> float:
    ...
    raw_value = os.getenv(LLM_TIMEOUT_ENV_VAR) or os.getenv(LEGACY_LLM_TIMEOUT_ENV_VAR)
...
def llm_max_attempts() -> int:
    ...
    raw_value = os.getenv(LLM_MAX_ATTEMPTS_ENV_VAR) or os.getenv(LEGACY_LLM_MAX_ATTEMPTS_ENV_VAR)
    if raw_value is None or not raw_value.strip():
...
def llm_retry_backoff_seconds() -> float:
    ...
    raw_value = os.getenv(LLM_RETRY_BACKOFF_ENV_VAR) or os.getenv(LEGACY_LLM_RETRY_BACKOFF_ENV_VAR)
    if raw_value is None or not raw_value.strip():

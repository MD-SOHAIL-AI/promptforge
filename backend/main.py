...
_GENERATION_EVENT_NAMES = {
    ...
    "project_validation_started": "PROJECT_VALIDATION_STARTED",
    "project_validation_failed": "PROJECT_VALIDATION_FAILED",
    "project_repair_started": "PROJECT_REPAIR_STARTED",
    "project_repair_completed": "PROJECT_REPAIR_COMPLETED",
    "project_repair_failed": "PROJECT_REPAIR_FAILED",
    "project_validation_completed": "PROJECT_VALIDATION_COMPLETED",
}
...
def _context_from_task(task: Task) -> ExecutionContext:
    ...
    for key in ("provider_id", "model_id", "fallback_provider_id", "fallback_enabled", "local_only"):
        if key not in context_metadata and key in workspace_metadata:
            context_metadata[key] = workspace_metadata[key]
...
class _WorkspaceAwarePlanner:
    ...
    def plan(self, prompt: str) -> ExecutionPlan:
        ...
        workspace = self._context.metadata.get("active_workspace")
        workspace_metadata = workspace if isinstance(workspace, Mapping) else {}
        routed_intent = _optional_text(workspace_metadata.get("agent_turn_intent"))
        requested_value = workspace_metadata.get("agent_requested_actions")
        requested_actions = {
            item.casefold()
            for item in requested_value
            if isinstance(item, str) and item.strip()
        } if isinstance(requested_value, (list, tuple)) else set()
        route_requires_generation = routed_intent == "generate" and "generate" in requested_actions
        next_task_type = TaskType.FIRMWARE_GENERATION if route_requires_generation else plan.task_type
        next_steps = list(plan.execution_steps)
        if route_requires_generation:
            next_steps = [step for step in next_steps if step is not ExecutionStep.GENERATE_CODE]
            next_steps.insert(0, ExecutionStep.GENERATE_CODE)
            if (
                workspace_metadata.get("agent_requires_build") is True
                or "build" in requested_actions
            ) and ExecutionStep.BUILD_FIRMWARE not in next_steps:
                next_steps.insert(1, ExecutionStep.BUILD_FIRMWARE)
        next_execution_steps = tuple(next_steps)
        if (
            next_board == target_board
            and next_framework == framework
            and next_task_type is plan.task_type
            and next_execution_steps == plan.execution_steps
        ):
            return plan
        metadata = {
            **dict(plan.metadata),
            ...
            "agent_turn_route_applied": route_requires_generation,
            "agent_turn_intent": routed_intent,
            "workflow": {
                **(
                    dict(plan.metadata.get("workflow", {}))
                    if isinstance(plan.metadata.get("workflow"), Mapping)
                    else {}
                ),
                "workspace_generation_needed": (
                    route_requires_generation and workspace_metadata.get("has_platformio_ini") is not True
                ),
            },
        }
        return ExecutionPlan(
            ...
            execution_steps=next_execution_steps,

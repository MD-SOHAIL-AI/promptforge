...
class CodeGenerationRequest:
    ...
    workspace_transaction: WorkspaceGenerationTransaction | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.workspace_transaction is not None and not isinstance(
            self.workspace_transaction, WorkspaceGenerationTransaction
        ):
            raise ValueError("workspace_transaction must be a WorkspaceGenerationTransaction")
...
class CodeGenerationService:
    ...
    def __init__(
        ...
        *,
        ...
        max_project_repair_attempts: int = 2,
        ...
        if (
            not isinstance(max_project_repair_attempts, int)
            or isinstance(max_project_repair_attempts, bool)
            or not 0 <= max_project_repair_attempts <= 2
        ):
            raise ValueError("max_project_repair_attempts must be an integer between 0 and 2")
        ...
        self.max_project_repair_attempts = max_project_repair_attempts
    ...
    async def generate_project(
        ...
        if selected_strategy == "chunked":
            ...
            owns_transaction = request.workspace_transaction is None
            transaction = request.workspace_transaction or (
            ...
            try:
                ...
                rolled_back = transaction.rollback() if transaction is not None and owns_transaction else False
                await _emit_generation_event(
                    request,
    ...
    async def repair_generated_project(
        self,
        request: CodeGenerationRequest,
        project: CanonicalGeneratedProject,
        validation_report: Mapping[str, object],
        *,
        attempt_number: int,
    ) -> CanonicalGeneratedProject:
        """Regenerate a complete project from a structured validation report."""

        if not 1 <= attempt_number <= self.max_project_repair_attempts:
            raise ProjectValidationError("project repair attempt is outside the configured limit")
        target_board = _target_board(request.plan.target_board)
        framework = _framework(request.plan.framework)
        current_files = [{"path": item.path, "content": item.content} for item in project.files]
        prompt = (
            "Repair this complete firmware project. Preserve every requested capability and do not silently remove user intent.\n"
            "Return the complete corrected project using the required JSON file schema.\n"
            f"Structured validation report:\n{json.dumps(_json_value(validation_report), ensure_ascii=True, sort_keys=True)}\n"
            f"Current project files:\n{json.dumps(current_files, ensure_ascii=True, sort_keys=True)}\n\n"
            + self._build_prompt(request, project_name=project.project_name)
        )
        attempts: list[GenerationAttempt] = []
        repaired = await self._generate_project_attempt(
            request,
        ...
        return GeneratedProject(
            ...
            target_board=repaired.target_board,
            framework=repaired.framework,
            ...
            metadata={
                ...
                "project_repair": {
                    "attempt_number": attempt_number,
                    "validation_report": _json_value(validation_report),
                },
            },
        )
    ...
    async def _generate_project_attempt(
        ...
        *,
        ...
        metadata.update(_llm_route_metadata(request.context.metadata))
            ...
            response = await self._generate_with_retries(
                LLMRequest(
                    ...
                    metadata={
                        ...
                        "allow_fallback": True,
                        "local_only": _metadata_bool(request.context.metadata, "local_only", False),
                        **_llm_route_metadata(request.context.metadata),
                    },
                ),
        ...
        *,
        ...
        for attempt_type, repair_used, fallback_used, service in (
            ...
            try:
                ...
                forbidden_value = summary.get("forbidden_features")
                forbidden_features = (
                    tuple(item for item in forbidden_value if isinstance(item, str))
                    if isinstance(forbidden_value, Sequence) and not isinstance(forbidden_value, (str, bytes, bytearray))
                    else ()
                )
                validation_errors = validate_chunk_file(
                    path,
                    content,
                    advanced=advanced,
...
def _llm_route_metadata(metadata: Mapping[str, Any]) -> dict[str, Any]:
    route: dict[str, Any] = {}
    provider_id = _metadata_text(metadata.get("provider_id"))
    model_id = _metadata_text(metadata.get("model_id"))
    fallback_provider_id = _metadata_text(metadata.get("fallback_provider_id"))
    if provider_id and provider_id not in {"verified_template", "fake_planner", "forgex_builtin", "forgex_local"}:
        route["provider_id"] = provider_id
    if model_id and provider_id not in {"verified_template", "fake_planner", "forgex_builtin", "forgex_local"}:
        route["model_id"] = model_id
    if fallback_provider_id:
        route["fallback_provider_id"] = fallback_provider_id
    return route
...
def _should_use_builtin_blink(
    ...
    if any(term in text for term in advanced_terms):
        return False
    selected_provider = _metadata_text(request.context.metadata.get("provider_id"))
    if selected_provider in {"verified_template", "forgex_builtin"}:
        return True

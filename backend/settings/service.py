...
class SettingsService:
    ...
    def get_settings(self) -> dict[str, Any]:
        ...
        for key, value in stored.items():
            if key not in SETTINGS_SCHEMA:
                continue
            try:
                merged[key] = self._normalize_value(key, value)
    ...
    def _normalize_value(self, key: str, value: Any) -> Any:
        ...
        if value_type == "select":
            if key == "appearance.motion" and value == "minimal":
                return "off"
        ...
        if value_type == "string":
            if isinstance(value, str):
                minimum_length = schema.get("min_length")
                maximum_length = schema.get("max_length")
                pattern = schema.get("pattern")
                if minimum_length is not None and len(value) < minimum_length:
                    raise SettingsValidationError(f"{key} is below minimum length")
                if maximum_length is not None and len(value) > maximum_length:
                    raise SettingsValidationError(f"{key} is above maximum length")

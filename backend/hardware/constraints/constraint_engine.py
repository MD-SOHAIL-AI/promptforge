...
class ConstraintEngine:
    ...
    def validate_constraints(
        ...
        *,
        ...
        selected_framework = framework or _extract_value(
            requirements, ("framework", "target_framework")
        )
...
def _boolean_capabilities(source: object) -> tuple[str, ...]:
    ...
    for candidate in candidates:
        ...
        for key, value in candidate.items():
            if value is True and isinstance(key, str) and key.casefold().endswith("_required"):
                result.append(_display_name(key[: -len("_required")]))

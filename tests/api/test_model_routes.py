    ...
    async def list_models(self) -> list[ModelInfo]:
        ...
        return [
            ModelInfo(
                ...
                free=True, supported_parameters=("tools", "tool_choice"),
            ),
            ModelInfo(
                "openrouter", "chat-only:free", "chat-only:free",
                free=True, supported_parameters=("temperature",),
            ),
        ]
...
def test_model_catalog_exposes_agent_capabilities_and_rejects_incompatible_selection(tmp_path: Path) -> None:
    with client(tmp_path) as api:
        catalog = api.get("/models/providers/openrouter/models")
        assert catalog.status_code == 200
        models = {item["model_id"]: item for item in catalog.json()["models"]}
        assert models["openai/gpt-oss-120b:free"]["agent_compatible"] is True
        assert models["openai/gpt-oss-120b:free"]["supported_parameters"] == ["tools", "tool_choice"]
        assert models["chat-only:free"]["agent_compatible"] is False

        rejected = api.put(
            "/models/selection",
            json={"provider_id": "openrouter", "model_id": "chat-only:free"},
        )

    assert rejected.status_code == 422
    assert rejected.json()["code"] == "MODEL_AGENT_CAPABILITY_UNSUPPORTED"

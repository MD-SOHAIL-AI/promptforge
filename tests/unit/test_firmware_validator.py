...
from backend.agent_runtime.templates import TEMPLATES
...
def test_reviewed_ssd1306_dependency_is_accepted() -> None:
    ini = VALID_INI + "lib_deps = adafruit/Adafruit SSD1306@^2.5.13\n"

    assert FirmwareValidator().validate_dependencies(project(ini=ini)).valid is True


@pytest.mark.parametrize("template", TEMPLATES, ids=lambda template: template.id)
def test_every_reviewed_agent_template_passes_firmware_policy(template: object) -> None:
    files = getattr(template, "files")
    framework = "PlatformIO" if any(path == "platformio.ini" for path, _ in files) else "Arduino"
    value = {
        "framework": framework,
    ...
    assert result.valid is True, tuple((issue.rule_id, issue.message) for issue in result.errors)

...
from backend.main import _WorkspaceAwarePlanner
...
def test_trusted_generate_route_cannot_be_downgraded_by_contextual_edit_words(planner: Planner) -> None:
    prompt = (
        "You may edit existing workspace files and prepare changes. "
        "User request: Build a complete ESP32 smart-room project."
    )
    assert planner.plan(prompt).task_type is TaskType.FIRMWARE_MODIFICATION
    context = ExecutionContext(
        task_id="task-routed-generate",
        project_path="C:/empty-workspace",
        target_board="ESP32",
        framework="PlatformIO",
        metadata={
            "active_workspace": {
                "has_platformio_ini": False,
                "agent_turn_intent": "generate",
                "agent_requested_actions": ["generate", "build"],
                "agent_requires_build": True,
            }
        },
    )

    plan = _WorkspaceAwarePlanner(planner, context).plan(prompt)

    assert plan.task_type is TaskType.FIRMWARE_GENERATION
    assert plan.execution_steps[:2] == (
        ExecutionStep.GENERATE_CODE,
        ExecutionStep.BUILD_FIRMWARE,
    )
    assert plan.metadata["agent_turn_route_applied"] is True
    assert plan.metadata["workflow"]["workspace_generation_needed"] is True


def test_planner_emits_typed_hardware_requirements_for_oled_intent(planner: Planner) -> None:
    plan = planner.plan("Create an OLED SSD1306 display for Arduino Uno")

    assert plan.metadata["hardware_requirements"] == {
        "peripherals": ("I2C",),

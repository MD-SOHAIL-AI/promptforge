...
from backend.services.generation_validators import validate_chunk_file
from backend.services.chunked_generation_writer import WorkspaceGenerationTransaction
...
def test_chunk_validator_maps_forbidden_oled_to_ssd1306_dependency() -> None:
    content = "[env:esp32dev]\nplatform=espressif32\nboard=esp32dev\nframework=arduino\nlib_deps=adafruit/Adafruit SSD1306@^2.5.13\n"

    errors = validate_chunk_file("platformio.ini", content, forbidden_features=("OLED",))

    assert errors == ("forbidden feature detected: oled",)


def test_chunk_validator_allows_requested_ssd1306_dependency() -> None:
    content = "[env:esp32dev]\nplatform=espressif32\nboard=esp32dev\nframework=arduino\nlib_deps=adafruit/Adafruit SSD1306@^2.5.13\n"

    assert validate_chunk_file("platformio.ini", content, forbidden_features=()) == ()
...
def test_shared_workspace_transaction_remains_rollbackable_after_generation(tmp_path: Path) -> None:
    workspace = tmp_path / "shared-transaction"
    workspace.mkdir()
    transaction = WorkspaceGenerationTransaction(workspace)
    llm = SequentialLLM([summary_json(), manifest_json(), platformio_ini(), config_h(), main_cpp(), readme_md()])
    service = CodeGenerationService(llm)
    request = CodeGenerationRequest(
        plan=plan(),
        context=context_at(workspace),
        workspace_transaction=transaction,
    )

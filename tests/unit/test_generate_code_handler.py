...
def test_internal_required_metadata_cannot_become_a_peripheral() -> None:
    result = run(
        GenerateCodeHandler(
            FakeGenerationService(),
            FakeProjectService(),
            ...
            plan(
                metadata={
                    "workflow": {"workspace_generation_needed": True},
                    "workspace_generation_required": True,
...
def test_explicit_unsupported_hardware_requirement_requires_user_decision() -> None:
    arduino_project = project(
        target_board="Arduino Uno",
        framework="Arduino",
        files=(GeneratedFile("blink.ino", "void setup() {}\nvoid loop() {}\n"),),
    )
    arduino_persisted = persisted(
        target_board="Arduino Uno",
        framework="Arduino",
        file_count=1,
    )
    result = run(
        GenerateCodeHandler(
            FakeGenerationService(arduino_project),
            FakeProjectService(arduino_persisted),
            context(target_board="Arduino Uno", framework="Arduino"),
        ).execute(
            plan(
                target_board="Arduino Uno",
                framework="Arduino",
                metadata={"hardware_requirements": {"peripherals": ["WiFi"]}},
            )
        )
    )

    assert result.status is GenerateCodeStatus.VALIDATION_FAILED
    assert result.metadata["pending_decision"] is True
    assert result.metadata["validation_report"]["requires_user_decision"] is True
    assert "ESP32 DevKit V1" in result.metadata["suggested_alternatives"]


def test_repairable_whole_project_validation_uses_bounded_repair() -> None:
    invalid = project(
        files=(
            GeneratedFile(
                "platformio.ini",
                "[env:esp32dev]\nplatform=espressif32\nboard=esp32dev\nframework=arduino\nlib_deps=unknown/UnsafeFirmwareLib@1.0\n",
            ),
            GeneratedFile("src/main.cpp", "#include <Arduino.h>\nvoid setup() {}\nvoid loop() {}\n"),
        )
    )

    class RepairingService(FakeGenerationService):
        max_project_repair_attempts = 2
        ...
        async def repair_generated_project(
            ...
            *,
            ...
            del request, generated, report, attempt_number
            self.repair_calls += 1
            return project()
    ...
    assert "project_repair_started" in {str(event["event_type"]) for event in events}
    assert "project_validation_completed" in {str(event["event_type"]) for event in events}


def test_whole_project_repair_failure_retries_twice_then_fails() -> None:
    invalid = project(
        files=(
            GeneratedFile(
                "platformio.ini",
    ...
    class FailingRepairService(FakeGenerationService):
        max_project_repair_attempts = 2
        ...
        async def repair_generated_project(self, *args: object, **kwargs: object) -> GeneratedProject:
            del args, kwargs
            self.repair_calls += 1
            raise RuntimeError("repair provider failed")

    events: list[dict[str, object]] = []
    service = FailingRepairService()
    result = run(
        GenerateCodeHandler(
            service,  # type: ignore[arg-type]
            FakeProjectService(),
    ...
    event_names = [str(event["event_type"]) for event in events]
    assert result.status is GenerateCodeStatus.VALIDATION_FAILED
    assert result.metadata["repair_attempt_count"] == 2
    assert service.repair_calls == 2
    assert event_names.count("project_repair_started") == 2
    assert event_names.count("project_repair_failed") == 2
